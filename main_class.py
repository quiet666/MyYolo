...
def run_classification_training(model, data):
    ...
    model = YOLO(f"/root/autodl-tmp/ultralytics-main/ultralytics/cfg/models/11/{model}.yaml").load(
        f"{model}.pt"
...
def run_classification_prediction(
    checkpoint, data, defect_class, predict_dir_name="predict_v3", confidence_threshold=0.9
):
    """封装分类任务的预测代码."""
    print("--- 运行分类预测 ---")
    model = YOLO(checkpoint)
    ...
    results = model.predict(
        f"{data}/val/{defect_class}", save=True, name=f"{predict_dir_name}/{defect_class}"
    )  # predict on an image
    ...
    if confidence_threshold > 0:
        print(
            f"低置信度的图片有 {low_confidence_count} 张，已保存至: {base_predict_path}/{defect_class}/low_confidence/"
        )
        print(
            f"低置信度且分类错误的图片有 {low_incorrect_count} 张，已保存至: {base_predict_path}/{defect_class}/low_incorrect/[错误类别名称]/"
        )
...
def simple_prediction(checkpoint, data, defect_class, predict_dir_name="predict", confidence_threshold=0.9):
    ...
    results = model.predict(
        f"{data}/{defect_class}", save=True, name=f"{predict_dir_name}/{defect_class}"
    )  # predict on an image
...
def main():
    ## 分类
    # 设置要操作的根目录
    data = r"/root/autodl-tmp/ultralytics-main/data/WX_class/v8/v8-binary_split"
